...
- **The job outlives the runner.** GitHub-hosted runners have time limits. A job that could exceed them cannot be run to completion in a single blocking step.
...
### The mistake I almost shipped: triggering on every merge, not every circuit change

I built the first version of `run-on-merge.yml` the obvious way: trigger on push to main, full stop. It ran, so I considered the expensive part solved and moved on to the accountability model in Part 5.

Someone asked a fair question later: are we paying for a quantum job every time someone merges a README fix? I had to actually check, and the honest answer was yes. The trigger did not know or care whether the circuit had changed — it fired on every push to main, because "push to main" was the only condition I had written down. A typo fix and a genuine circuit change looked identical to the workflow.

That is a cost problem, but it is also a governance problem, and the second one worries me more. The prod environment's required-reviewer gate exists so a human looks at "should this specific run happen" before real money moves. If that approval request shows up for merges that obviously have nothing to do with the circuit, reviewers learn — correctly, rationally — that most of these requests carry no signal, and start approving on reflex. The gate is still technically satisfied. It has quietly stopped doing its job.

The fix is a `paths:` filter on the trigger, scoped to the circuit payload directory and the small set of pipeline files that decide what actually gets submitted. Spend should track circuit changes, not merge cadence. I would call this a design bug, not an edge case — I simply had not asked "what should *not* cause this to run" until someone else did.
...
*A gap I found inside this same point.* I originally treated "who approved the spend" as the whole approver story. It is not. The environment's required-reviewer setting lives in Settings → Environments, not in the repository — nothing in a diff tells you who is allowed to approve a prod run, or when that list last changed; you need admin access and have to go looking. It answers "should this specific execution spend money right now," asked at run time, after the circuit has already merged. What was missing was an answer to the earlier question: who is accountable for having actually looked at the circuit *before* it merged at all. Ordinary branch protection can be satisfied by any reviewer with write access, which is not the same as "a named, quantum-literate person vouched for this diff." I added a `CODEOWNERS` file scoping the circuit and pipeline paths to specific named owners, with branch protection set to require their review. The difference that matters: `CODEOWNERS` is a checked-in file. Its own changes go through review. Every approval it requires is attached permanently to the pull request, visible to anyone reading that PR later — not buried in an admin-only settings page. So the model is now two gates, not one, answering two different questions: `CODEOWNERS` says a specific named person is accountable for this diff being sound; the environment's required reviewers say a specific named person authorized this specific dollar amount to go out right now. Keep both — collapsing them back into "well, someone approved it somewhere" is the same mistake this section opened by warning against, just one level up.

> **Margin question.** Is there prior art for splitting "who reviewed the change" from "who authorized the spend" as two separate, differently-scoped approval gates outside quantum/CI contexts — release engineering, financial systems? I suspect this pattern already has a name I don't know.
...
- **Separate "who reviewed the change" from "who authorized the spend."** They are different questions asked at different times. A checked-in `CODEOWNERS` file makes the first one visible in every PR, permanently — an environment's required-reviewer setting, config-only and admin-gated, should not be asked to carry both jobs alone.

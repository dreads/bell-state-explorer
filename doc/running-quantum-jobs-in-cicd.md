...
Blocking breaks down along predictable lines, worth naming so the wall is visible before hitting it:
...
The thing rarely discussed is almost the inverse, and it's far more mundane. It's not "quantum breaks our crypto." It's: **a quantum-cloud credential is just an API credential with a compute resource and a dollar-denominated blast radius attached, and CI/CD is the thing holding it.** Quantum compute gets wired into delivery pipelines with roughly the same casual trust as a linter — and unlike a linter, *nobody approving the job can read what it does.* The circuit is opaque to the reviewer. The cost of a fumbled approval isn't a bad deploy rolled back in minutes; it's real spend against a credential, executing whatever the circuit actually encoded, on hardware nobody owns or can inspect.

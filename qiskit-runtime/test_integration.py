...
def main() -> int:
    ...
    # Loading from a raw token (not qiskit_ibm_runtime's saved-account file)
    # is deliberate -- the token comes from QISKIT_IBM_TOKEN, a CI secret,
    # never from a local account file. The library itself emits a WARNING
    # log line to this effect (_discover_account) -- that is expected, not
    # a bug, and is left visible in CI output on purpose.
    ...
    # Pin the specific device we're monitoring -- unlike run.py's hardware
    # path, this deliberately does NOT fall back to least_busy: the nightly
    # check's whole point is tracking one named device's health over time.
